...
double Mediana(const list<int>& lst) {
...
void sortAndOutputStudents(list<Studentas>& studentai) {
    ...
    list<Studentas> vargsai, kietakai;
    for (const auto& student : studentai) {
        double vidurkis = 0.0;
        if (!student.nd.empty()) {
            for (const auto& grade : student.nd) {
    ...
    vargsai.sort(sortFunction);
    kietakai.sort(sortFunction);
    ...
    do {
        ...
        if (pasirinkimas == 't') {
            ...
            for (int j = 0; j < n; j++, ++it) {
                *it = std::rand() % 10 + 1; // Generuojame balus nuo 1 iki 10
            }
...
void handleFileInput(list<Studentas>& studentai) {
 ...
 cout << "Pasirinkite strategija (1 - du konteineriai, 2 - vienas konteineris su trynimu, 3 - naudojant splice): ";
    int strat;
    cin >> strat;
    if (strat == 1) {
        sortAndOutputStudents(studentai); // 1 strategija
    } else if (strat == 2) {
        skaidyti2_list(studentai);      // 2 strategija
    }else if (strat == 3) { 
    skaidyti3_list(studentai);
    }else {
...
void skaidyti3_list(list<Studentas>& studentai) {
    ...
    for (auto it = studentai.begin(); it != studentai.end(); ) {
        ...
        double galutinis = 0.4 * vidurkis + 0.6 * it->egz;
        if (galutinis < 5.0) {
            vargsai.splice(vargsai.end(), studentai, it++); // Greitai perkelia elementą į vargsai
        } else {
            ++it;
        }
    }
    ...
    auto end = high_resolution_clock::now();
    cout << "Skirstymo i dvi grupes trukme (splice): " << duration_cast<duration<double>>(end - start).count() << " s\n";
    cout << "Vargsai: " << vargsai.size() << ", Kietakai: " << studentai.size() << endl;
